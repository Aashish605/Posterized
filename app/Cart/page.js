...
const Cart = () => {
    ...
    return (
        <main className="px-8 py-10 bg-white min-h-screen">
            <h1 className="text-4xl  font-semibold mb-8 text-left mx-auto ">Shopping Cart</h1>
            ...
            <div className="mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <section className="lg:col-span-8 bg-white border border-gray-300 shadow-xs rounded-2xl p-6">
                    <div className="flex  justify-between mb-6">
                        <div className="text-[1.1rem] font-semibold max-[530px]:mx-auto  mx-5 ">Products</div>
                        <div className="flex  max-[530px]:hidden gap-8 ">
                            <div className="w-32 max-[700px]:hidden text-[1.1rem] font-semibold text-center">Quantity</div>
                            <div className="w-32 text-[1.1rem] font-semibold text-right">Total</div>
                            <div className="w-12" />
                        </div>
                    </div>
                        ...
                        {cartItems.length === 0 ? (
                            ...
                            cartItems.map(item => (
                                <div key={item.id} className="flex max-[530px]:items-start items-center gap-4 p-4 border-t-2 border-gray-500   hover:shadow-xs transition">
                                    <img src={item.url} alt={item.name} className=" max-w-20  max-h-40 object-cover rounded-xl " />
                                    <div className="flex-1">
                                        <div className="">{item.name}</div>
                                        {item.sizpri.split('|')[1]
                                        ?<div className="text-gray-500 text-sm">Size: {item.sizpri.split('|')[1]}</div>:""}
                                        ...
                                        <div className='flex gap-4   items-center my-3'>
                                            <div className="w-8  font-semibold  min-[530px]:hidden ">
                                                {`Rs ${(Number(item.sizpri.split('|')[0]) * item.quantity).toFixed(2)}`}
                                            </div>
                    ...
                    <div className="bg-white border border-gray-300 shadow-xs rounded-lg p-6 sticky top-16 max-[550px]:relative max-[550px]:top-0">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm text-gray-600">
                                ...
                                <span>
                                    {`Rs ${cartItems
                                        .reduce((s, i) => s + (`${Number(i.sizpri.split('|')[0])}` || 0) * i.quantity, 0)
                                        .toFixed(2)}`}
                                </span>
                            </div>
